...
export const fetchProjectData = async (projectId: string): Promise<ProjectData> => {
    try {
        ...
        const response = await fetch(`${origin}/api/scan-project?path=${encodeURIComponent(projectId)}`);
        if (!response.ok) {
        ...
        if (hasScript && !hasVO) {
            ...
            try {
                ...
                if (!generated) {
                    await fetch(`${origin}/api/generate-vo`, {
                        method: 'POST',
                    ...
                    const retry = await fetch(`${origin}/api/scan-project?path=${encodeURIComponent(projectId)}`);
        ...
        if (hasVO) {
            try {
                ...
                if (subAsset) {
                    const subRes = await fetch(`${origin}${subAsset.path}`);
                    if (subRes.ok) {
        ...
        try {
            ...
            if (scriptPath) {
                ...
                const scriptRes = await fetch(`${origin}${scriptPath}`);
        ...
        for (const asset of project.assets) {
            if (asset.type === 'video' && !asset.originalDuration) {
                try {
                    const fullUrl = asset.path.startsWith('http') ? asset.path : `${origin}${asset.path}`;
        ...
        if (project.voiceoverTrack) {
            try {
                const voUrl = `${origin}${project.voiceoverTrack}`;
